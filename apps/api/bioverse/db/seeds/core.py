"""Core demo tenant: Northside Health, its clinicians, patients, history and review queue.

Runs once. Later modules add their own data in sibling files, each idempotent.
"""
...
from bioverse.db.seeds.ids import *  # noqa: F403  (fixed demo IDs)


def already_seeded(conn) -> bool:
    return conn.execute("SELECT 1 FROM organizations WHERE id = %s", (ORG,)).fetchone() is not None  # noqa: F405
